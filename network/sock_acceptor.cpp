...
// class SOCK_Acceptor
SOCK_Acceptor::SOCK_Acceptor(Net_Manager *net_manager, void* pUserData, bool useBigQueue)
{
...
int SOCK_Acceptor::create_tcp_server(const INET_Addr& local_addr, int netbufsize)
{
    ...
    {
        CnError("SOCK_Acceptor::create_tcp_server error, socket error, errno:%d\n", error_no());
        return -1;
    }
    ...
    if (nret != 0)
    {
        CnError("SOCK_Acceptor::set_socket_nonblocking error, socket error, errno:%d\n", error_no());
        return -1;
    }
    ...
    if (0 != rc) 
    {
        CnError("SOCK_Acceptor::create_tcp_server error, bind error, errno:%d\n", error_no());
        close(m_socket);
        m_socket = INVALID_SOCKET;
        return -1;
    }
    ...
    if (0 != rc) 
    {
        CnError("SOCK_Acceptor::create_tcp_server error, listen error, errno:%d\n", error_no());
        close(m_socket);
        m_socket = INVALID_SOCKET;
        return -1;
    }
...
int SOCK_Acceptor::handle_input()
{
    while (true)
    {
        ...
        if (-1 == accept_socket)
        {
			if (NETWORK_EAGAIN == error_no() || NETWORK_EWOULDBLOCK == error_no())
            {
                return 0;
            }
            else 
        ...
        SOCK_Stream *stream = new SOCK_Stream(new_id, m_net_manager, accept_socket, m_local_addr, remote_addr, m_UserData, m_use_big_queue );
