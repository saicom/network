...
uint32_t Net_Manager::create_udp(const INET_Addr& local_addr, void* pUserData, int netbufsize)
{
    if (1 != m_status)
    {
        CnWarn("Net_Manager::create_udp error, not started\n");
        return 0;
    }
    ...
    if (0 != rc) 
    {
        CnWarn("Net_Manager::create_udp error, create_udp error\n");
        CnDelete dgram;
        return 0;
    }
...
uint32_t Net_Manager::create_tcp_client(const INET_Addr& remote_addr, void* pUserData, int timeout, bool useBigQueue, int netbufsize)
{
    if (1 != m_status) 
    {
    ...
    SOCK_Connector *connector = CnNew SOCK_Connector(this, pUserData, useBigQueue );
                                        ...
                                        void* pUserData, int timeout, bool useBigQueue,  int netbufsize)
{
    ...
    return create_tcp_client(remote_addr, pUserData, timeout, useBigQueue, netbufsize);
}

uint32_t Net_Manager::create_tcp_server(const INET_Addr& local_addr, 
                                        void* pUserData, bool useBigQueue, int netbufsize)
{
    ...
    SOCK_Acceptor *acceptor = CnNew SOCK_Acceptor(this, pUserData, useBigQueue );
    uint32_t id = acceptor->get_id();

    rc = acceptor->create_tcp_server(local_addr, netbufsize);
    if (0 != rc) 
    {
        CnWarn("Net_Manager::create_tcp_server error, create_tcp_server error\n");
        CnDelete acceptor;
        return 0;
    }
...
uint32_t Net_Manager::create_tcp_server(const char *local_ip, int local_port,
                                        void* pUserData, bool useBigQueue, int netbufsize)
{
    INET_Addr local_addr;
    local_addr.set_addr(inet_addr(local_ip));
    ...
    return create_tcp_server(local_addr, pUserData, useBigQueue, netbufsize);
}
