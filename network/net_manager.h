...
public:
    ...
    //! @param netbufsize 网络底层缓冲区大小
    //! @return 通道id, >0:成功, 0失败
    uint32_t create_tcp_client(const INET_Addr& remote_addr, 
    ...
    //! @param netbufsize 网络底层缓冲区大小
    //! @return 通道id, >0:成功, 0失败
    uint32_t create_tcp_client(const char *remote_ip, int remote_port, 
        void* pUserData, int timeout, bool useBigQueue, int netbufsize = NETBUF_SIZE);
    ...
    //! @param netbufsize 网络底层缓冲区大小
    //! @return 通道id, >0:成功, 0失败
    uint32_t create_tcp_server(const INET_Addr& local_addr, void* pUserData, bool useBigQueue, int netbufsize = NETBUF_SIZE);
    ...
    //! @param netbufsize 网络底层缓冲区大小
    //! @return 通道id, >0:成功, 0失败
    uint32_t create_tcp_server(const char *local_ip, int local_port, void* pUserData, bool useBigQueue, int netbufsize = NETBUF_SIZE);
