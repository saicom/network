...
// class SOCK_Stream
SOCK_Stream::SOCK_Stream(uint32_t id, Net_Manager *net_manager, SOCKET socket, const INET_Addr& local_addr, 
...
int SOCK_Stream::handle_input()
{
    while (true)
    {
        ...
        if (rc > 0)
        {
			...
			int head_len = m_packet_splitter->head_size();

            // 循环拆包
            while (true)
            {
                packet_len = 0;
                int split_result = m_packet_splitter->split(m_recv_buffer.rd_ptr(), m_recv_buffer.data_length(), msg_id, packet_len);
                if (1 == split_result)
                {
					...
					net_event->packet.msg_id = msg_id;
					net_event->packet.msg_len = packet_len;
                    net_event->packet.write_data(m_recv_buffer.rd_ptr(), packet_len);
                    m_net_manager->put_event(net_event);

                    m_recv_buffer.rd_ptr(packet_len);
                }
                else if (0 == split_result)
                {
                    // no packet in buf
                    if(packet_len > (uint32_t)m_recv_buffer.length())
                    {
						...
						m_PartDataEvent->packet.msg_id = msg_id;
						m_PartDataEvent->packet.msg_len = packet_len;
                ...
                {
                    // buff is empty, no packet in buff
                    break;
                }
            }

            if ((m_recv_buffer.data_length() == 0) || (m_recv_buffer.space_length() == 0)
                || (uint32_t)(m_recv_buffer.data_length() + m_recv_buffer.space_length()) < packet_len)
            {
                m_recv_buffer.recycle();
            }
...
int SOCK_Stream::post_packet(Net_Packet* send_packet)
{
    if (m_current_send_packet != NULL)
    {
        ...
        {
            // 队列用尽
            CnError("SOCK_Stream::send_data error, send task queue is full, packet lost, net_id%d, ip:%s, port:%u\n",
                m_id, GetIpStr(m_remote_addr.get_addr()).c_str(), m_remote_addr.get_port());
            // 连接关闭
...
int SOCK_Stream::realSend()
{
    ...
    int nDataLen = pNetPacket->msg_len - m_current_send_length;
    int rc = 0;
    ...
    while(nDataLen > 0)
    {
        pBuf = pNetPacket->get_rptr();
        rc = send(m_socket, pBuf, pNetPacket->cur_len, 0);
        if (rc > 0) 
        {
            // 发送成功
            m_current_send_length += rc;
            pNetPacket->read_data(rc);
            nDataLen -= rc;
        }
        else 
        {
    ...
    assert(m_current_send_length == pNetPacket->msg_len);
    delete m_current_send_packet;
    m_current_send_packet = NULL;
    m_current_send_length = 0;
    return 0;
