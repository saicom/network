...
#define TIME_START  (DATE_START + 11)
    ...
    int Log::set_file_name(const char *filename)
    {
        ...
        strncpy(file_name_, filename, MAX_PATH_LEN);
        return 0;
    }
    ...
    int Log::vlog(int level, const char * fmt, va_list ap)
    {
        ...
        if(enable_usec)
        {
            ...
            snprintf(((char*)level_str_usec_[level]+TIME_START), 64, "%02d:%02d:%02d.%06ld",					
        ...
        {
            snprintf(((char*)level_str_[level]+TIME_START), 64, "%02d:%02d:%02d",					
