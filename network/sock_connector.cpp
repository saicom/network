...
SOCK_Connector::SOCK_Connector(Net_Manager *net_manager, void* pUserData, bool useBigQueue )
{
...
int SOCK_Connector::connect_server(const INET_Addr& remote_addr, int timeout)
{
    ...
    if ((rc != 0) && (error_no() != NETWORK_EINPROGRESS)) 
    {
...
int SOCK_Connector::handle_output()
{
    ...
    // create stream
    SOCK_Stream *stream = new SOCK_Stream(m_id, m_net_manager, m_socket, local_addr, m_remote_addr, m_UserData, m_use_big_queue );
