...
int Select_Reactor::run_reactor_event_loop()
{
    ...
    if(get_handler_count() == 0)     // 无socket
    {
        Sleep(10);
        return 0;
    }
	...
	for (auto it = m_eventHandlerMap.begin(); it != m_eventHandlerMap.end(); ++it) {
		Event_Handler* pEventHandler = it->second;
	...
	for (auto it = m_eventHandlerMap.begin(); it != m_eventHandlerMap.end(); ) {
