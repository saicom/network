...
int SOCK_Dgram::realSend()
{
    ...
    int nDataLen = pNetPacket->cur_len - m_current_send_length;
    int rc = 0;
    ...
    while(nDataLen > 0)
    {
        ...
        rc = sendto(m_socket, pBuf, pNetPacket->cur_len, 0, (sockaddr*)&to, tolen);
        if (rc > 0) {
            nDataLen -= rc;
